<template>
  <div>
    <template v-if="!state.chatMode">
      <div class="flex gap-md q-mb-sm items-center" style="font-size: 10px">
        <q-chip dense size="10px" :label="`TOTAL: ${currentCountSMS.length}`" />
        |
        <q-chip
          dense
          size="10px"
          :label="`MENSAGENS: ${currentCountSMS.messages}`"
        />
        |
        <q-chip
          dense
          size="10px"
          :label="`RESTANTE: ${currentCountSMS.remaining}`"
        />
      </div>

      <q-field label="Mensagem" outlined stack-label :disable="disableChat">
        <editor-content
          v-if="editorInstance"
    ...
    <div v-if="!hideChatMode" class="flex justify-end">
...
watch(
  () => props.modelValue,
  (newValue) => {
    if (editorInstance.value && newValue !== editorInstance.value.getText()) {
      editorInstance.value.commands.setContent(newValue, false)
      handleInfo(editorInstance.value, newValue)
    }
  },
)
</script>
